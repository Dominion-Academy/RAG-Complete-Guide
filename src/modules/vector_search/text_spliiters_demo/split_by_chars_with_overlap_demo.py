...
from src.components.similarity import calculate_cosine_similarity
from src.components.text_splitters import split_by_chars_with_overlap
...
for max_chars, overlap in product([128, 256, 512, 1024], [0, 64]):
    print(f"max_chars={max_chars}, overlap={overlap}")

    chunks = split_by_chars_with_overlap(text, max_chars=max_chars, overlap=overlap)
    for idx, chunk in enumerate(chunks):
        chunk_embeddings = embeddings_model.embed_document([chunk])
        similarity = calculate_cosine_similarity(query_embedding, chunk_embeddings[0])
        print(idx, len(chunk), round(similarity, 2))
